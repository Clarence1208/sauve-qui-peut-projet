...
pub fn init_logging(log_dir: &str, categories: &[&str]) -> Result<(), std::io::Error> {
    ...
    match LOG_MAP.set(Mutex::new(new_map)) {
        ...
        Err(_) => {
            ...
            if let Some(mutex_map) = LOG_MAP.get() {
                let mut global_map = mutex_map.lock().map_err(|_| {
...
fn write_separator(path: String, file: &mut File, metadata: Metadata) -> Result<(), Error> {
    if metadata.len() > 0 {
        file.seek(SeekFrom::End(0))?;
        let separator = "\n\n\n########## NEW SESSION ##########\n";
        if let Err(e) = file.write_all(separator.as_bytes()) {
            error!("Failed to write separator to '{}': {}", path, e);
        }
    }
    Ok(())
}
    ...
    if let Some(mutex_map) = LOG_MAP.get() {
        ...
        if let Some(file) = map.get_mut(category) {
            ...
            if let Err(e) = writeln!(file, "{}", message) {
                error!("Failed writing to '{}.log': {}", category, e);
            }
        } else {
...
mod tests {
    ...
    #[test]
    fn test_init_logging_creates_directory() -> Result<(), std::io::Error> {
    ...
    #[test]
    fn test_init_logging_creates_files_for_categories() -> Result<(), std::io::Error> {
        let temp_dir = tempdir()?;
        let log_dir = temp_dir.path().join("test");
        let log_dir_str = log_dir.to_str().unwrap();
    ...
    #[test]
    fn test_init_logging_appends_separator_for_existing_file() -> Result<(), std::io::Error> {
        let temp_dir = tempdir()?;
        let log_dir = temp_dir.path().join("test");
        let log_dir_str = log_dir.to_str().unwrap();
    ...
    #[test]
    fn test_init_logging_doesnt_append_separator_for_new_file() -> Result<(), std::io::Error> {
        let temp_dir = tempdir()?;
        let log_dir = temp_dir.path().join("test");
        let log_dir_str = log_dir.to_str().unwrap();
    ...
    #[test]
    fn test_init_logging_merges_new_categories() -> Result<(), std::io::Error> {
        let temp_dir = tempdir()?;
        let log_dir = temp_dir.path().join("test");
        let log_dir_str = log_dir.to_str().unwrap();
    ...
    #[test]
    fn test_log_message_appends_text() -> Result<(), std::io::Error> {
        let temp_dir = tempdir()?;
        let log_dir = temp_dir.path().join("test");
        let log_dir_str = log_dir.to_str().unwrap();
